...
class NTWMweightedVertex:
    ...
    def read(self, reader):
        ...
        counts_data = reader.read_bytes(4)
        self.weight_count = struct.unpack("<4B", counts_data)[0]
        if self.weight_count == 0:
            self.weight_count = 1
        
        weights_data = reader.read_bytes(16)
        self.weights.extend(struct.unpack("<4f", weights_data)[:self.weight_count])
        
        bone_data = reader.read_bytes(4)
        self.boneIndexes.extend(struct.unpack("<4B", bone_data)[:self.weight_count])
...
class NTWMMesh:
    def __init__(self, hasWeights = False):
        self.vertices = []
        self.face_indexes = []
        self.name = ""
        self.vertex_count = 0
        self.face_count = 0
        self.weightedVertexes = [] 
        self.hasWeights = hasWeights        
    
    def read_vertex_weights(self, reader):
        for i in range(self.vertex_count):
            weightedVertex = NTWMweightedVertex()
            weightedVertex.read(reader)

            self.weightedVertexes.append(weightedVertex)           
...
class FXMModel:
    ...
    def read(self, filepath):
        ...
        self.unk = reader.read_uint()
        self.boneNum = reader.read_uint()
        if self.boneNum:
        ...
        for _ in range(self.mesh_count):
            mesh = NTWMMesh(self.boneNum)
            mesh.read(reader)
            self.meshes.append(mesh)
    
    def read_bones(self, reader):
        for _ in range(self.boneNum):
            bone = NTWMBone()
            bone.read(reader)
    ...
    def write(self, filepath):
        ...
        writer.write_uint(1 if self.type == 1 else 0)
