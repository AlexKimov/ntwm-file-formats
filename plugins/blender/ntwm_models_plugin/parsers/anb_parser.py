...
class NTWMFrameMesh:
    ...
    def write(self, writer):
        positions = vector3f_list_to_bytes(self.positions)
        writer.write_bytes(positions)
        normals = vector3f_list_to_bytes(self.normals)
        writer.write_bytes(normals)
...
class ANBModel:
    ...
    def makeFirstFrameFromMeshes(self):
        # first frame will be model mesh itself
        morph_frame = NTWMMorphFrame()
        for mesh in self.meshes:
            frame_mesh = NTWMFrameMesh().from_mesh(mesh)
    ...
    def read(self, filepath):
        ...
        # first frame (0) will be model mesh itself
        self.makeFirstFrameFromMeshes()
        
        for i in range(self.morph_frame_count - 1):           
            morph_frame = NTWMMorphFrame()
            for mesh in self.meshes:
